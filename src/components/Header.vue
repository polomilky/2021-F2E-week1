  ...
  export default {
    ...
    methods: {
      ...
      getSelectCountry(e) {
        ...
        // 儲存所選縣市
        sessionStorage.setItem('locationName', this.title.locationName);
      },
      // 取得類型
      getSelectType(e) {
      ...
      search() {
        if (!this.selectCountey) {
          ...
          this.$store.commit({
            type: 'setIsSearchTaiwanData',
            isSearchTaiwanData: false,
          });
          this.$store.commit({
            type: 'setSearchCountryData',
            searchCountryData: this.selectCountey,
          });
        }
        this.axios.get(
          `https://ptx.transportdata.tw/MOTC/v2/Tourism/${this.selectItem}/${this.selectCountey}?$top=6&$format=JSON`,
          {
          ...
          .then((response) => {
            ...
            this.$emit('listenToChildEvent', this.getSearchData, this.selectItem);
          })
          .catch((error) => {
      ...
      searchKey() {
        if (this.keywordText !== null) {
          ...
          if (!keyword) {
            ...
            if (this.selectCountey === null) {
              // 恢復沒有選擇縣市區域, 查看更多：無跳頁, 每次多秀三個
              // store vuex
              this.$store.commit({
                type: 'setIsSearchTaiwanData',
                isSearchTaiwanData: true,
              });
              this.$emit('listenToChildKeywordEvent', keyword, '');
            } else {
              this.$emit('listenToChildKeywordEvent', keyword, this.selectCountey);
            }
          }
        } else {
          return false;
